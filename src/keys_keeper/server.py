...
class _AdminRequestHandler(BaseHTTPRequestHandler):
    ...
    def _serve_new(self) -> None:
        ...
        html = render_new_edit(paths=self.paths, token=self.admin.token)
        self._send(200, html.encode("utf-8"))

    def _serve_bulk_paste(self) -> None:
        from keys_keeper.pages import render_bulk_paste

        html = render_bulk_paste(paths=self.paths, token=self.admin.token)
        self._send(200, html.encode("utf-8"))
    ...
    def _serve_entry_path(self, path: str) -> None:
        ...
        entry = self._find_entry(unquote(suffix))
        if entry is None:
            self._send(404, b"entry not found")
            return
        if edit:
            self._serve_entry_edit(entry)
            return
        self._serve_entry_detail(entry)

    def _find_entry(self, identifier: str):
        from keys_keeper.store import MetadataStore

        store = MetadataStore(self.paths)
        return store.get_by_id(identifier) or store.get_by_name(identifier)

    def _serve_entry_edit(self, entry) -> None:
        from keys_keeper.pages import render_new_edit

        html = render_new_edit(paths=self.paths, token=self.admin.token, entry=entry)
        self._send(200, html.encode("utf-8"))

    def _serve_entry_detail(self, entry) -> None:
